...
(function () {
  ...
  function load() {
    ...
    try {
      if (!merged.user || !(merged.user.name && merged.user.email && merged.user.designation)) {
        ...
        const name = (wn.vlab_exp8_user_name || "").toString().trim();
        const email = (wn.vlab_exp8_user_email || "").toString().trim();
        const designation = (wn.vlab_exp8_user_designation || "").toString().trim();
        if (name && email && designation) {
          merged.user = { name, email, designation, submittedAt: wn.vlab_exp8_user_submitted_at || nowISO() };
        }
      }
    } catch {}
  ...
  function setUser(user) {
    ...
    setWindowNameValues({
      vlab_exp8_user_name: trimmedUser.name,
      vlab_exp8_user_email: trimmedUser.email,
      vlab_exp8_user_designation: trimmedUser.designation,
      vlab_exp8_user_submitted_at: state.user.submittedAt
    });
  ...
  function resetAll() {
    ...
    try {
      const wn = loadWindowNameData();
      delete wn.vlab_exp8_user_name;
      delete wn.vlab_exp8_user_email;
      delete wn.vlab_exp8_user_designation;
      delete wn.vlab_exp8_user_submitted_at;
