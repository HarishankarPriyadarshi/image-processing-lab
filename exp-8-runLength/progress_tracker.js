...
(function () {
  const KEY = "vlab_exp8_progress_v1";
  const VERSION = 1;

  const nowISO = () => new Date().toISOString();

  const GENERAL_PROGRESS_KEYS = [
    "vlab_exp8_pretest_score",
    "vlab_exp8_pretest_total",
    "vlab_exp8_pretest_attempted_ids",
    "vlab_exp8_pretest_correct_ids",
    "vlab_exp8_pretest_updated_at",
    "vlab_exp8_posttest_score",
    "vlab_exp8_posttest_total",
    "vlab_exp8_posttest_attempted_ids",
    "vlab_exp8_posttest_correct_ids",
    "vlab_exp8_posttest_updated_at",
    "vlab_exp8_simulation_report_html",
    "vlab_exp8_simulation_report_updated_at"
  ];
  const USER_PROGRESS_SUFFIXES = GENERAL_PROGRESS_KEYS.map((key) => key.replace(/^vlab_exp8_/, ""));
  const RESET_LOCAL_STORAGE_KEYS = [
    "vlab_exp8_user_input_draft"
  ];
  const RESET_SESSION_STORAGE_KEYS = [
    "vlab_exp8_current_page",
    "vlab_exp8_page_enter_ms",
    "vlab_exp8_prompted_once"
  ];

  const WINDOW_NAME_PREFIX = "VLAB_EXP8::";
  ...
  function load() {
    ...
    try {
      if (!merged.user || !(merged.user.name && merged.user.email && merged.user.designation)) {
        ...
        const name = (wn.vlab_exp_user_name || "").toString().trim();
        const email = (wn.vlab_exp_user_email || "").toString().trim();
        const designation = (wn.vlab_exp_user_designation || "").toString().trim();
        if (name && email && designation) {
          merged.user = { name, email, designation, submittedAt: wn.vlab_exp_user_submitted_at || nowISO() };
        }
      }
    } catch {}
  ...
  function hasSimulationReport() {
    try {
      ...
      const keys = [];
      if (activeHash) keys.push(`vlab_exp8_user_${activeHash}_simulation_report_html`);
      keys.push("vlab_exp8_simulation_report_html");
      for (const k of keys) {
  ...
  function initPage() {
    ...
    try {
      sessionStorage.setItem("vlab_exp8_current_page", p);
      sessionStorage.setItem("vlab_exp8_page_enter_ms", String(Date.now()));
    } catch {}
  }
  ...
  function recordPageExit() {
    ...
    const p = (() => {
      try { return sessionStorage.getItem("vlab_exp8_current_page") || pageName(); }
      catch { return pageName(); }
    })();

    let enterMs = null;
    try {
      const s = sessionStorage.getItem("vlab_exp8_page_enter_ms");
  ...
  function removeUserScopedProgress(userHash) {
    if (!userHash) return;
    const prefix = `vlab_exp8_user_${userHash}_`;
    try {
  ...
  function setUser(user) {
    ...
    let prevHash = "";
    try { prevHash = localStorage.getItem("vlab_exp_active_user_hash") || ""; } catch {}
    ...
    try {
      if (newHash) localStorage.setItem("vlab_exp_active_user_hash", newHash);
      else localStorage.removeItem("vlab_exp_active_user_hash");
    } catch {}
    ...
    setWindowNameValues({
      vlab_exp_user_name: trimmedUser.name,
      vlab_exp_user_email: trimmedUser.email,
      vlab_exp_user_designation: trimmedUser.designation,
      vlab_exp_user_submitted_at: state.user.submittedAt
    });
  ...
  function resetAll() {
    let activeHash = "";
    try { activeHash = localStorage.getItem("vlab_exp_active_user_hash") || ""; } catch {}

    // remove main state
    try { localStorage.removeItem(KEY); } catch {}

    // remove active hash
    try { localStorage.removeItem("vlab_exp_active_user_hash"); } catch {}
    ...
    try {
      const wn = loadWindowNameData();
      delete wn.vlab_exp_user_name;
      delete wn.vlab_exp_user_email;
      delete wn.vlab_exp_user_designation;
      delete wn.vlab_exp_user_submitted_at;
      delete wn.vlab_exp8_simulation_report_html;
      delete wn.vlab_exp8_simulation_report_updated_at;
